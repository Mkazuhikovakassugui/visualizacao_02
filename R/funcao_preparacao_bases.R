#------------------------------------------------------------------------------------------------------------#
#                                   Função Preparação da base ENEM <ANO>
...
# As seguintes operações serão realizadas:
    ...
    ## elimina os valores missing excluindo os alunos treineiros, os ausentes nas provas e os eliminados;
...
preparacao_base <- function(ano,uf) {
    ...
    endereco <- paste0("data/enem_", ano, ".csv")
    ...
    # Os valores da variável presença assume para cada prova - 0 se faltou, 1 se presente, 2 se eliminado.
    ...
    # seis = o aluno compareceu em um dos dias (1+1) e foi eliminado nas duas provas do segundo dia (2+2)
    # cinco = o aluno compareceu em um dos dias (1+1) e foi eliminado em uma prova do segundo dia (1+2)
    ...
    # Para usar esta lógica, devemos converter os valores das variáveis categóricas da seguinte forma:
    # Transformar - "Faltou à prova"      para 0
    #             - "Presente na prova"   para 1
    #             - "Eliminado na prova"  para 2

    enem_uf <- enem_uf |>
        dplyr::mutate(dplyr::across(                 # aplicamos a função de fórmula nas colunas com across().
            .cols = dplyr::starts_with("tp_presenca"),
            .fns = ~ dplyr::case_when(.x == "Faltou à prova" ~ 0,
                                      .x == "Presente na prova" ~ 1,
                                      .x == "Eliminado na prova" ~ 2)
        ))
    ...
    # Caso o aluno tenha participado do primeiro dia (1+1) e no segundo dia tenha sido eliminado em uma prova
    # e não participado da segunda prova (2 + 0), também somaria 4. Então, podemos eliminar estes valores.
