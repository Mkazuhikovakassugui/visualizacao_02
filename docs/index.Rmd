...
```{r label = 'grafico_08'}
#| fig.cap = "Gráfico 08 - gráfico de colunas das notas de linguagens e códigos por intervalos"
...
```{r echo=FALSE}
#| fig.cap = "Gráfico 10 - Gráfico donnut - línguas estrangeiras",
#| fig.align = "center",
#| fig.height = 8
...
### Evolução das notas de Linguagens e Códigos nos últimos anos 
...
```{r label = 'gráfico_11'}
#| fig.cap = "Gráfico 11 - Gráfico de linhas - evolução das notas de linguagens e códigos",
#| fig.height = 10,

p11 <- enem_lc_historico |>
  ggplot2::ggplot() +
  ggplot2::aes(
    x = nu_ano,
    y = media,
...
```{r label = 'gráfico_12'}
#| fig.align = "center",
#| fig.cap = "Gráfico 12 - BoxPlot e histograma da distribuição de notas de ciências da natureza",
...
```{r label = 'grafico_013'}
#| fig.cap = "Gráfico 13 - gráfico de colunas das notas de ciências da naturez por intervalo."

p13 <- tab_classes_notas_cn |>
  ggplot2::ggplot() +
  ggplot2::aes(
    x = Var1,
    y = Freq,
...
#| fig.cap = "Gráfico 14 - Gráfico de colunas - médias por dependência administrativa na prova de Ciências da Natureza",
#| fig.align = "center"

p14 <- notas_tipo_escola |>
  ggplot2::ggplot() +
  ggplot2::aes(
    x = forcats::fct_reorder(
      tp_dependencia_adm_esc,
...
```{r label = 'gráfico_16'}
#| fig.cap = "Gráfico 16 - Gráfico de linhas - evolução das notas de Ciências da Natureza",
#| fig.height = 10,
...
```{r label = 'gráfico_17'}
#| fig.align = "center",
#| fig.cap = "Gráfico 17 - BoxPlot e histograma da distribuição de notas de ciências humanas",

## gráfico_17_1 - boxplot das notas 
p17_1 <- enem_cidade |>
...
```{r label = 'grafico_18'}
#| fig.cap = "Gráfico 18 - Diagrama de colunas das notas de Ciências Humanas por intervalos"

p18 <- tab_classes_notas_ch |>
  ggplot2::ggplot() +
  ggplot2::aes(
    x = Var1,
    y = Freq,
...
```{r label = 'gráfico_19_medias_dependencia_adm_ciencias_humanas'}
#| fig.align = "center"

p19 <- notas_tipo_escola_ch |>
  ggplot2::ggplot() +
  ggplot2::aes(
    x = forcats::fct_reorder(
      tp_dependencia_adm_esc,
...
```{r label = 'gráfico_20'}
#| fig.cap = "Gráfico 20 - Gráfico de linhas - evolução das notas de CH",
#| fig.height = 10,

p20 <- enem_ch_historico |>
  ggplot2::ggplot() +
  ggplot2::aes(
    x = nu_ano,
    y = media,
...
```{r label = 'gráfico_21'}
#| fig.align = "center",
#| fig.cap = "Gráfico 20 - BoxPlot e histograma da distribuição de notas de Matemática",
## gráfico_17_1 - boxplot das notas 
p21_1 <- enem_cidade |>
  dplyr::select(nu_nota_mt) |>
  ggplot2::ggplot(ggplot2::aes(x = nu_nota_mt)) +
  ggplot2::geom_boxplot(
    fill = "#11B5E4",
    alpha = 0.7
  ) +
...
# gráfico do histograma das notas de matemática (parte inferior da visualização)
p21_2 <- enem_cidade |>
  dplyr::select(nu_nota_mt) |>
  ggplot2::ggplot(ggplot2::aes(x = nu_nota_mt)) +
  ggplot2::geom_histogram(
...
```{r label = 'notas_máximas_mt'}
#| echo = FALSE

nota_max_mt <- enem_cidade |>
  dplyr::summarize(nota_max = max(nu_nota_mt))

nota_min_mt <- enem_cidade |>
  dplyr::summarise(nota_min = min(nu_nota_mt))

nota_media_mt <- enem_cidade |>
  dplyr::summarise(nota_media_mt = mean(nu_nota_mt, na.rm = TRUE))

```

```{r label = 'base_grafico_freq_relativa_intervalo_de_classes_mt'}
#| echo = FALSE,
#| fig.align = "center"

fabs_notas_mt <- enem_cidade |>
  dplyr::select(nu_nota_mt) |>
  table()

# total de alunos com nota máxima

qtd_nota_max_mt <- fabs_notas_mt |>
  tail(n = 1)

# criar as classes de frequências das notas

intervalo_classes_notas_mt <- seq(0, 1000, 100)

tab_classes_notas_mt <- table(
  cut(enem_cidade$nu_nota_mt,
    breaks = intervalo_classes_notas_mt,
    right = FALSE
  )
) |>
  prop.table() |>
...
maiores_classes_notas_mt <- tab_classes_notas_mt |>
  dplyr::arrange(desc(Freq)) |>
  dplyr::slice_max(order_by = Freq, n = 2) |>
  tibble::tibble()


menores_classes_notas_mt <- tab_classes_notas_mt |>
  dplyr::arrange(desc(Var1)) |>
  dplyr::filter(Freq != 0) |>
  dplyr::slice_tail() |>
  tibble::tibble()
...
A maior nota obtida no município de `r params$cidade` de **`r nota_max_mt`** foi obtida por **`r qtd_nota_max_mt`** aluno(s) e a nota média foi de **`r round(nota_media_mt,2)`**.

<br>

### Distribuição das Notas de Matemática por Intervalo de Classes.

```{r label = 'grafico_22'}
#| fig.cap = "Gráfico 22 - Diagrama de colunas das notas de Matemática por intervalos"

p22 <- tab_classes_notas_mt |>
  ggplot2::ggplot() +
  ggplot2::aes(
    x = Var1,
    y = Freq,
...
No gráfico 21, pode-se visualizar como as notas desta área de conhecimento se distribuíram por intervalos de notas. Perceba que **`r round(sum(maiores_classes_notas_mt["Freq"]),3)`%** dos participante(s), ou seja, **`r format((sum(maiores_classes_notas_mt["Freq"]) * total_alunos_municipio)/100, scientific = FALSE)`** aluno(s) obtiveram notas nos intervalos **`r maiores_classes_notas_ch[["Var1"]][2]`** e **`r maiores_classes_notas_mt[["Var1"]][1]`**. Com relação às menores notas, tem-se que  **`r round(menores_classes_notas_mt["Freq"],3)`%** ou **`r (menores_classes_notas_mt["Freq"] * total_alunos_municipio)/100`** aluno(s) obtiveram notas no intervalo de **`r menores_classes_notas_mt[["Var1"]]`**.

<br>

## Médias das Notas por Dependência Administrativa

```{r label = 'base_grafico_medias_dep_adm_matematica'}
#| fig.cap = "Média por tipo de dependência administrativa",
#| echo = FALSE

notas_tipo_escola_mt <- enem_cidade |>
  dplyr::select(tp_dependencia_adm_esc, nu_nota_mt) |>
  tidyr::drop_na() |>
  dplyr::group_by(tp_dependencia_adm_esc) |>
  dplyr::summarise(medias = mean(nu_nota_mt, na.rm = TRUE)) |>
  dplyr::arrange(desc(medias))

```

```{r label = 'gráfico_23_medias_dependencia_adm_matematica'}
#| fig.align = "center"

p23 <- notas_tipo_escola_mt |>
  ggplot2::ggplot() +
  ggplot2::aes(
    x = forcats::fct_reorder(
      tp_dependencia_adm_esc,
...
### Evolução das Notas de Matemática nos Últimos Anos 

```{r label = 'base_grafico_24'}
#| echo = FALSE

## filtrar a base para a cidade
enem_mt_historico <- enem |>
  dplyr::filter(no_municipio_prova == params$cidade) |>
  dplyr::select(nu_nota_mt, nu_ano) |>
  dplyr::group_by(nu_ano) |>
  dplyr::summarise(media = mean(nu_nota_mt, na.rm = TRUE))

```


```{r label = 'gráfico_24'}
#| fig.cap = "Gráfico 24 - Gráfico de linhas - evolução das notas de MT",
#| fig.height = 10,

p24 <- enem_mt_historico |>
  ggplot2::ggplot() +
  ggplot2::aes(
    x = nu_ano,
    y = media,
  ...
  ggplot2::scale_y_continuous(limits = c(
    min(enem_mt_historico$media) - 50,
    max(enem_mt_historico$media) + 50
  ))
...
Neste trabalho, foi criado um modelo de relatório que permite a produção de documentos para os oitenta e oito municípios paranaenses, nos quais os exame do ENEM foi realizado. 

Esta ferramenta permite visualizar informações geradas a partir dos microdados do ENEM que refletem o desempenho dos candidatos por área de conhecimento. 

Com os dados por município pode-se ter uma visão geral das áreas com melhor e pior desempenho. A análise das médias das notas do exame permite à administração pública a implementação de programas, em suas respectivas localidades, de melhorias que possibilitem a todas as escolas, sejam públicas ou privadas, federal, estadual ou municipal a obtenção de um ensino com o mesmo padrão, reduzindo a diferença nos desempenhos que podem ser observados nos relatórios gerados por este instrumento.
