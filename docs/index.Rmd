...
O modelo deve ter a característica de reprodutibilidade, de modo que relatórios (formato, textos e visualizações) sejam produzidos a partir de uma rotina de procedimentos (*script*) de forma automatizada para todos os 88 municípios do estado do Paraná, nos quais as provas foram realizadas.
...
dados_geobr |>
  ...
  ggplot2::theme(legend.title = ggplot2::element_blank(),
                 legend.background = ggplot2::element_rect(fill = "#EBF9FF",
                                                           colour = "#EBF9FF"))
...
p1_1 <- enem_cidade |> 
  ...
  ggplot2::geom_boxplot(fill = "#11B5E4",
                        alpha = 0.7
                        )+
  ggplot2::labs(
...
```{r label = 'gráfico_07'}
#| fig.align = "center",
#| fig.cap = "Gráfico 07 - BoxPlot e histograma da distribuição de notas de ciências da natureza",
...
## gráfico_08_1 - boxplot das notas 
p7_1 <- enem_cidade |> 
...
A área de conhecimento "Ciências Humanas e suas Tecnologias" é composta pelas disciplinas de química, física e biologia. 
...
```{r label = 'grafico_08'}
#| fig.cap = "Gráfico_08_diagrama_colunas_notas_CN_por_intervalos"
...
No gráfico 08, pode-se visualizar como as notas desta área de conhecimento se distribuíram por intervalos de notas. Perceba que **`r round(sum(maiores_classes_notas_cn["Freq"]),3)`%** dos participante(s), ou seja, **`r (sum(maiores_classes_notas_cn["Freq"]) * total_alunos_municipio)/100`** aluno(s) obtiveram notas nos intervalos **`r maiores_classes_notas_cn[["Var1"]][2]`** e **`r maiores_classes_notas_cn[["Var1"]][1]`**. Com relação às menores notas, temos que  **`r round(menores_classes_notas_cn["Freq"],3)`%** ou **`r (menores_classes_notas_cn["Freq"] * total_alunos_municipio)/100`** aluno(s) obtiveram notas no intervalo de **`r menores_classes_notas_cn[["Var1"]]`**.
